...
namespace DevTreks.Data
{
    ...
    ///             220: .ToLower() added for uriparts because netcore3.0 started returning with capitals
    public class ContentURI
    {
            ...
            if (routeParams != null)
            {
                ...
                for (i = 0; i < routeParams.Length; i++)
                {
                    if (i == 0)
                    {
                        ...
                        controller = controller.Replace("*", string.Empty).ToLower();
                    }
                    else if (i == 1)
                    {
                        action = routeParams[i];
                        action = action.Replace("*", string.Empty).ToLower();
                    }
                    else if (i == 2)
                    {
                        networkName = routeParams[i];
                        networkName = networkName.Replace("*", string.Empty).ToLower();
                    }
                    else if (i == 3)
                    {
                        nodeName = routeParams[i];
                        nodeName = nodeName.Replace("*", string.Empty).ToLower();
                    }
                    else if (i == 4)
                    {
                        name = routeParams[i];
                    ...
                    else if (i == 6)
                    {
                        ...
                        fileNameExtensionType = fileNameExtensionType.Replace("*", string.Empty).ToLower();
                    }
                    else if (i == 7)
                    {
                        subAction = routeParams[i].ToLower();
                    }
                    else if (i == 8)
                    {
                        subActionView = routeParams[i].ToLower();
                    }
                    else if (i == 9)
                    {
                        variable = routeParams[i].ToLower();
                    }
                }
            }
        }
        ...
        public static string GetURIPatternPart(string uriPattern, string patternPartName)
        {
            ...
            {
                sURIPatternPart = GetURIPatternPart(uriPattern, URIPATTERNPART.fileExtension).ToLower();
            }
            ...
            {
                sURIPatternPart = GetURIPatternPart(uriPattern, URIPATTERNPART.network).ToLower();
            }
            else if (patternPartName == URIPATTERNPART.node.ToString())
            {
                sURIPatternPart = GetURIPatternPart(uriPattern, URIPATTERNPART.node).ToLower();
            }
            return sURIPatternPart;
        }
        public static string GetContentURIPatternPart(string uriPattern, string patternPartName)
        {
            ...
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.controller).ToLower();
            }
            else if (patternPartName == CONTENTURIPATTERNPART.action.ToString())
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.action).ToLower();
            }
            else if (patternPartName == CONTENTURIPATTERNPART.network.ToString())
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.network).ToLower();
            }
            else if (patternPartName == CONTENTURIPATTERNPART.node.ToString())
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.node).ToLower();
            }
            ...
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.fileExtension).ToLower();
            }
            else if (patternPartName == CONTENTURIPATTERNPART.subaction.ToString())
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.subaction).ToLower();
            }
            else if (patternPartName == CONTENTURIPATTERNPART.subactionview.ToString())
            {
                sURIPatternPart = GetContentURIPatternPart(uriPattern, CONTENTURIPATTERNPART.subactionview).ToLower();
            }
            ...
            return sURIPatternPart.ToLower();
        }
