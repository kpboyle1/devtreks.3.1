...
namespace DevTreks.ViewModels
{
    ...
    public class ContentViewModel 
    {
            ...
            string partialUriPattern)
        {
            ...
            await SetContentURIResourceAsync(contentService);
            //220 added this additional autorization check
            if (IsAuthorizedForSubAction(ContentURIData))
            {
        ...
        //    //return asyncOp;
        //}
        private static bool IsAuthorizedForSubAction(ContentURI uri)
        {
            bool bIsAuthorizedForSubAction
                    = (uri.URIMember.ClubInUse.PrivateAuthorizationLevel
                    == AccountHelper.AUTHORIZATION_LEVELS.fulledits)
                    ? true : false;
            if (uri.URIDataManager.ServerSubActionType 
                == DataHelpers.SERVER_SUBACTION_TYPES.makepackage
                || uri.URIDataManager.ServerSubActionType
                == DataHelpers.SERVER_SUBACTION_TYPES.runaddin)
            {
                //no authorization needed for packaging content
                //or looking at a calculator
                bIsAuthorizedForSubAction = true;
            }
            return bIsAuthorizedForSubAction;
        }
